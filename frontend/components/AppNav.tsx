...
import { Home, CalendarDays, Trophy, GitMerge, BarChart2 } from "lucide-react";

const NAV_ITEMS = [
  { href: "/", label: "Hoje", icon: Home },
  { href: "/calendar", label: "Agenda", icon: CalendarDays },
  { href: "/pickem", label: "Pick'em", icon: GitMerge },
...
export function AppNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 flex justify-center">
      <div className="w-full max-w-lg bg-brand-green/95 backdrop-blur-md border-t border-[#1E4A32]">
          ...
          {NAV_ITEMS.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <span className="absolute -top-px w-8 h-0.5 bg-brand-gold rounded-full" />
                )}
              </Link>
            );
          })}
